...
int test_realloc(vector_t *int_vect, vector<int> &int_sol)
{
    ...
    // doubles as a test for add back after regular add (ensures that add
    // updates the length)
...
int test_size(vector_t *string_vect, vector<char *> &string_sol)
{
    ...
    if (vector_size(string_vect) == string_sol.size()) {
        ...
        printf(
            "Failed test: Size Test. Expected size = %lu. Actual size = %lu\n",
...
int main()
{
    ...
    // points system or just by visually looking at the code and making sure
    // they call free
