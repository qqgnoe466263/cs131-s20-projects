...
public:
    ...
    /**
     * Stops the server. Cleans up any threads and resources the server
     * allocated, and returns the final balances of all clients.
     *
...
private:
    ...
    // Thread that receives requests from clients and puts them on the work
    // queue.
     ...
     * @param client: Pointer we will set to client's account pointer when
     * found.
     ...
     * pointer when found.
     * @return 0 on success, appropriate error if failed (-ECLINOTFOUND if
     * origin client not found, -ETARNOTFOUND if target client not found, or
     * -ESELFACTION if origin and target clients are the same).
     */
    int get_two_accounts(uint64_t first_id,
                         account_t **first_account,
                         uint64_t second_id,
                         account_t **second_account);
     ...
     * @param n: Pointer to a notification_t struct to send to the target
     * client.
     * @return 0 on success, -ECLINOTFOUND if client not found.
     */
     ...
     * client(s) are missing for the given request. Writes appropriate response
     * to the original client, if it's found.
    ...
    /**
     * Helper function that handles a client that is reconnecting. Merges the
     * new client info into existing client_conn_t that we already have in the
     * `client_conns` map.
     *
     * @param new_client: New client_conn_t for the client that is reconnecting.
     * @param existing_client: Existing client_conn_t that we already have in
     * our `client_conns` map for the same client.
     */
 ...
 * return -1 like other handlers).
 */
int handle_charge(request_t *req,
                  account_t *first_account,
                  account_t *second_account,
                  response_t *resp,
                  notification_t *notif);
