...
void Server::work_loop()
{
    // TODO: implement
    // While the server is still running (i.e., `is_stopped` is still false),
    // pop a request off the server's work queue in a loop, and call
    // `process_request` to process the request.
...
void Server::handle_missing_clients(int err, request_t *req)
{
    ...
    if (get_account(req->origin_client_id, &first_account) < 0)
        return;
...
void print_request(request_t *req)
{
    ...
    case PAYMENT:
        printf("[PAYMENT] Client %ld $%ld to target %ld\n",
               req->origin_client_id, req->amount, req->target_client_id);
        break;
    case CHARGE:
